...
<script lang="ts">
  ...
  import {QUERY_GET_CARGO_PAGING} from "@/applications/onei/domain/ipc/components/nomenclador/graphql/graphql-nomenclador";

  export default {
    name: "GenericNomEditPage",
    components: {
    ...
    data() {
      return {
        getQ: resolveQuery(this.$route.params.type, this.$route.params.id),
        loading: true,
        nomenclador: {
          id: -1,
          nombre: '',
    ...
    created() {
      //this.fetch();
      //this.$apollo.queries.nomenclador.refetch();
    },
    apollo: {
      nomenclador: {
        query(): any {
          return this.getQ.query.query
        },
        variables(): any {
          return this.getQ.query.variables;
        },
        update(data: any): any {
          debugger;
          const nom = {...data[this.getQ.data]};
          this.loading = false;
