      ...
      <template slot="items" slot-scope="props">
        ...
        <td class="text-md-center justify-center layout px-0">
          <v-menu bottom
            ...
            <v-list>
              <v-list-tile :to="`/onei/sigip/ipc/nomenclador/editar/tipo/tipologia/${props.item.id}`">
                <v-list-tile-action>
                  <v-icon>edit</v-icon>
                </v-list-tile-action>
                <v-list-tile-title>
              ...
              <v-list-tile :to="`/onei/sigip/ipc/nomenclador/view/tipo/tipologia/${props.item.id}`">
                <v-list-tile-action>
...
<script lang="ts">
  ...
  import {
    QUERY_GET_TYPOLOGY_PAGING,
    SEARCH_TYPOLOGY_BY_DESCRIPTION,
    TOTAL_SEARCH_TYPOLOGY_BY_DESCRIPTION
  ...
  export default {
    name: "onei-nomenclador-list-typology",
    components: {
      NoFoundResult,
      VAlert,
      VContainer,
    ...
    apollo: {
      nomenclators: {
        query: QUERY_GET_TYPOLOGY_PAGING,
        variables: {
          page: 0,
          size: 5
        },
        update(data) {
          return data.typologiesPaging
        }
      },
      totalNomenclators: {
        query: gql`query{
        ...
        update(data) {
          return data.totalTypology
        }
      },
      nomenclatorsFilter: {
        query: SEARCH_TYPOLOGY_BY_DESCRIPTION,
        variables: {
          description: "null",
          page: 0,
          size: 5
        ...
        update(data) {
          return data.typologyByDescriptionContains
        }
      },
      totalNomenclatorsFilter: {
        query: TOTAL_SEARCH_TYPOLOGY_BY_DESCRIPTION,
        variables: {
          description: "null",
        },
        update(data) {
          return data.totalTypologyByDescriptionContains
        }
      }
    },
    watch: {
        ...
        if (this.filterModel.nombre !=='') {
          return new Promise((resolve, reject) => {
            ...
            this.$apollo.queries.nomenclatorsFilter.fetchMore({
              ...
              updateQuery: (previousResult, {fetchMoreResult}) => {
                const items = fetchMoreResult.typologyByDescriptionContains;
                this.loading = false;
                resolve({
                  items
                })
          ...
          return new Promise((resolve, reject) => {
            ...
            this.$apollo.queries.nomenclators.fetchMore({
              ...
              updateQuery: (previousResult, {fetchMoreResult}) => {
                const items = fetchMoreResult.typologiesPaging;
                this.loading = false;
                resolve({
                  items
                })
