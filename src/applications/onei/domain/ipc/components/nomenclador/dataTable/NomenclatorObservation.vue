      ...
      <template slot="items" slot-scope="props">
        ...
        <td class="text-md-center justify-center layout px-0">
          <v-menu bottom
            ...
            <v-list>
              <v-list-tile :to="`/onei/sigip/ipc/nomenclador/editar/tipo/observacion/${props.item.id}`">
                <v-list-tile-action>
                  <v-icon>edit</v-icon>
                </v-list-tile-action>
                <v-list-tile-title>
              ...
              <v-list-tile :to="`/onei/sigip/ipc/nomenclador/view/tipo/observacion/${props.item.id}`">
                <v-list-tile-action>
...
<script lang="ts">
  ...
  import {
    QUERY_GET_OBSERVATION_PAGING,
    SEARCH_OBSERVATION_BY_DESCRIPTION,
    TOTAL_SEARCH_OBSERVATION_BY_DESCRIPTION
...
import {filterModelProperty} from "@shared/mixins";
  ...
  export default {
    name: "onei-nomenclador-list-observation",
    ...
    apollo: {
      nomenclators: {
        query: QUERY_GET_OBSERVATION_PAGING,
        variables: {
          page: 0,
          size: 5
        },
        update(data) {
          return data.observationsPaging
        }
      },
      totalNomenclators: {
        query: gql`query{
        ...
        update(data) {
          return data.totalObservation
        }
      },
      nomenclatorsFilter: {
        query: SEARCH_OBSERVATION_BY_DESCRIPTION,
        variables: {
          description: "null",
          page: 0,
          size: 5
        ...
        update(data) {
          return data.observationByDescriptionContains
        }
      },
      totalNomenclatorsFilter: {
        query: TOTAL_SEARCH_OBSERVATION_BY_DESCRIPTION,
        variables: {
          description: "null",
        },
        update(data) {
          return data.totalObservationByDescriptionContains
        }
      }
    },
    watch: {
        ...
        if (this.filterModel.nombre !== '') {
          return new Promise((resolve, reject) => {
            ...
            this.$apollo.queries.nomenclatorsFilter.fetchMore({
              ...
              updateQuery: (previousResult, {fetchMoreResult}) => {
                const items = fetchMoreResult.observationByDescriptionContains;
                this.loading = false;
                resolve({
                  items
                })
          ...
          return new Promise((resolve, reject) => {
            ...
            this.$apollo.queries.nomenclators.fetchMore({
              ...
              updateQuery: (previousResult, {fetchMoreResult}) => {
                const items = fetchMoreResult.observationsPaging;
                this.loading = false;
                resolve({
                  items
                })
