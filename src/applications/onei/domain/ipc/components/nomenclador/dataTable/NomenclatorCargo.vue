      ...
      <td class="text-md-center justify-center layout px-0">
        <v-menu bottom
          ...
          <v-list>
            ...
            <v-list-tile :to="`/onei/sigip/ipc/nomenclador/editar/tipo/cargo/${props.item.id}`">
              <v-list-tile-action>
                <v-icon>edit</v-icon>
              </v-list-tile-action>
              <v-list-tile-title>
            ...
            <v-list-tile :to="`/onei/sigip/ipc/nomenclador/view/tipo/cargo/${props.item.id}`">
              <v-list-tile-action>
...
<script lang="ts">
  ...
  import {
    QUERY_GET_CARGO_PAGING,
    SEARCH_CARGO_BY_DESCRIPTION,
    TOTAL_SEARCH_CARGO_BY_DESCRIPTION
  ...
  export default {
    name: "onei-nomenclador-list-cargo",
    ...
    apollo: {
      nomenclators: {
        query: QUERY_GET_CARGO_PAGING,
        variables: {
          page: 0,
          size: 5
        },
        update(data) {
          return data.cargosPaging
        }
      },
      totalNomenclators: {
        query: gql`query{
        ...
        update(data) {
          return data.totalCargos
        }
      },
      nomenclatorsFilter: {
        query: SEARCH_CARGO_BY_DESCRIPTION,
        variables: {
          descripcion: "null",
          page: 0,
          size: 5
        ...
        update(data) {
          return data.cargoByDescription
        }
      },
      totalNomenclatorsFilter: {
        query: TOTAL_SEARCH_CARGO_BY_DESCRIPTION,
        variables: {
          descripcion: "null",
        },
        update(data) {
          return data.totalCargoByDescription
        }
      }
    },
    watch: {
        ...
        if (this.filterModel.nombre !== '') {
          return new Promise((resolve, reject) => {
            ...
            this.$apollo.queries.nomenclatorsFilter.fetchMore({
              ...
              updateQuery: (previousResult, {fetchMoreResult}) => {
                const items = fetchMoreResult.cargoByDescription;
                this.loading = false;
                resolve({
                  items
                })
          ...
          return new Promise((resolve, reject) => {
            ...
            this.$apollo.queries.nomenclators.fetchMore({
              ...
              updateQuery: (previousResult, {fetchMoreResult}) => {
                const items = fetchMoreResult.cargosPaging;
                this.loading = false;
                resolve({
                  items
                })
