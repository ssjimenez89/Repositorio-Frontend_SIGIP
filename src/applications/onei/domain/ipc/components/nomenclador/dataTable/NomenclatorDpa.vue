      ...
      <template slot="items" slot-scope="props">
        ...
        <td class="text-md-center justify-center layout px-0">
          <v-menu bottom
            ...
            <v-list>
              <v-list-tile :to="`/onei/sigip/ipc/nomenclador/editar/tipo/dpa/${props.item.id}`">
                <v-list-tile-action>
                  <v-icon>edit</v-icon>
                </v-list-tile-action>
                <v-list-tile-title>
              ...
              <v-list-tile :to="`/onei/sigip/ipc/nomenclador/view/tipo/dpa/${props.item.id}`">
                <v-list-tile-action>
...
<script lang="ts">
  ...
  import {
    QUERY_GET_DPA_PAGING,
    SEARCH_DPA_BY_DESCRIPTION,
    TOTAL_SEARCH_DPA_BY_DESCRIPTION
  ...
  export default {
    name: "onei-nomenclador-list-dpa",
    ...
    apollo: {
      nomenclators: {
        query: QUERY_GET_DPA_PAGING,
        variables: {
          page: 0,
          size: 5
        },
        update(data) {
          return data.dpasPaging
        }
      },
      totalNomenclators: {
        query: gql`query{
        ...
        update(data) {
          return data.totalDpa
        }
      },
      nomenclatorsFilter: {
        query: SEARCH_DPA_BY_DESCRIPTION,
        variables: {
          description: "null",
          page: 0,
          size: 5
        ...
        update(data) {
          return data.dpaByDescriptionContains
        }
      },
      totalNomenclatorsFilter: {
        query: TOTAL_SEARCH_DPA_BY_DESCRIPTION,
        variables: {
          description: "null",
        },
        update(data) {
          return data.totalDpaByDescriptionContains
        }
      }
    },
    watch: {
        ...
        if (this.filterModel.nombre != '') {
          return new Promise((resolve, reject) => {
            ...
            this.$apollo.queries.nomenclatorsFilter.fetchMore({
              ...
              updateQuery: (previousResult, {fetchMoreResult}) => {
                const items = fetchMoreResult.dpaByDescriptionContains;
                this.loading = false;
                resolve({
                  items
                })
          ...
          return new Promise((resolve, reject) => {
            ...
            this.$apollo.queries.nomenclators.fetchMore({
              ...
              updateQuery: (previousResult, {fetchMoreResult}) => {
                const items = fetchMoreResult.dpasPaging;
                this.loading = false;
                resolve({
                  items
                })
