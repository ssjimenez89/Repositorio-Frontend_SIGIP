      ...
      <template slot="items" slot-scope="props">
        ...
        <td class="text-md-center justify-center layout px-0">
          <v-menu bottom
            ...
            <v-list>
              <v-list-tile :to="`/onei/sigip/ipc/nomenclador/editar/tipo/tipo_unidad_medida/${props.item.id}`">
                <v-list-tile-action>
                  <v-icon>edit</v-icon>
                </v-list-tile-action>
                <v-list-tile-title>
              ...
              <v-list-tile :to="`/onei/sigip/ipc/nomenclador/view/tipo/tipo_unidad_medida/${props.item.id}`">
                <v-list-tile-action>
...
<script lang="ts">
  ...
  import {
    QUERY_GET_MEASUREMENTUNITTYPE_PAGING,
    SEARCH_MEASUREMENTUNITTYPE_BY_DESCRIPTION,
    TOTAL_SEARCH_MEASUREMENTUNITTYPE_BY_DESCRIPTION
  ...
  export default {
    name: "onei-nomenclador-list-tipo_unidad_medida",
    ...
    apollo: {
      nomenclators: {
        query: QUERY_GET_MEASUREMENTUNITTYPE_PAGING,
        variables: {
          page: 0,
          size: 5
        },
        update(data) {
          return data.measurementUnitTypesPaging
        }
      },
      totalNomenclators: {
        query: gql`query{
          totalMeasurementUnitType
      }`,
        update(data) {
          return data.totalMeasurementUnitType
        }
      },
      nomenclatorsFilter: {
        query: SEARCH_MEASUREMENTUNITTYPE_BY_DESCRIPTION,
        variables: {
          description: "null",
          page: 0,
          size: 5
        ...
        update(data) {
          return data.measurementUnitTypeByDescriptionContains
        }
      },
      totalNomenclatorsFilter: {
        query: TOTAL_SEARCH_MEASUREMENTUNITTYPE_BY_DESCRIPTION,
        variables: {
          description: "null",
        },
        update(data) {
          return data.totalMeasurementUnitTypeByDescriptionContains
        }
      }
    },
    watch: {
        ...
        if (this.filterModel.nombre !== '') {
          return new Promise((resolve, reject) => {
            ...
            this.$apollo.queries.nomenclatorsFilter.fetchMore({
              ...
              updateQuery: (previousResult, {fetchMoreResult}) => {
                const items = fetchMoreResult.measurementUnitTypeByDescriptionContains;
                this.loading = false;
                resolve({
                  items
                })
          ...
          return new Promise((resolve, reject) => {
            ...
            this.$apollo.queries.nomenclators.fetchMore({
              ...
              updateQuery: (previousResult, {fetchMoreResult}) => {
                const items = fetchMoreResult.measurementUnitTypesPaging;
                this.loading = false;
                resolve({
                  items
                })
