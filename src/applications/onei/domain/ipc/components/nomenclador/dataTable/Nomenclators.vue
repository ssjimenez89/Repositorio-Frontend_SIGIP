<template>
  ...
  >
    <template slot="items" slot-scope="props">
      ...
      <td class="text-xs-left">{{ $t('applications.onei_ipc.modules.nomenclador.types.'+props.item.type) }}</td>
      ...
      <td class="text-md-center justify-center layout px-0">
        <v-menu bottom
          ...
          <v-list>
            <v-list-tile
              :to="`/onei/sigip/ipc/nomenclador/editar/tipo/${props.item.type}/${props.item.identificador}`">
              <v-list-tile-action>
                <v-icon>edit</v-icon>
              </v-list-tile-action>
              <v-list-tile-title>
            ...
            <v-list-tile :to="`/onei/sigip/ipc/nomenclador/view/tipo/${props.item.type}/${props.item.identificador}`">
              <v-list-tile-action>
...
<script lang="ts">
  ...
  import {
    QUERY_GET_NOMENCLATORS_PAGING,
    SEARCH_NOMENCLATORS_BY_DESCRIPTION,
    TOTAL_SEARCH_NOMENCLATORS_BY_DESCRIPTION
  ...
  export default {
    ...
    apollo: {
      nomenclators: {
        query: QUERY_GET_NOMENCLATORS_PAGING,
        variables: {
          page: 0,
          size: 5
        },
        update(data) {
          return data.nomenclatorsPaging
        }
      },
      totalNomenclators: {
        query: gql`query{
          totalAllNomenclators
      }`,
        update(data) {
          return data.totalAllNomenclators
        }
      },
      nomenclatorsFilter: {
        query: SEARCH_NOMENCLATORS_BY_DESCRIPTION,
        variables: {
          description: "null",
          page: 0,
          size: 5
        ...
        update(data) {
          return data.nomenclatorsByDescriptionContains
        }
      },
      totalNomenclatorsFilter: {
        query: TOTAL_SEARCH_NOMENCLATORS_BY_DESCRIPTION,
        ...
        update(data) {
          return data.totalAllNomenclatorsByDescriptionContains
        }
      }
    },
    watch: {
        ...
        if (this.filterModel.nombre !== '') {
          return new Promise((resolve, reject) => {
            ...
            this.$apollo.queries.nomenclatorsFilter.fetchMore({
              ...
              updateQuery: (previousResult, {fetchMoreResult}) => {
                const items = fetchMoreResult.nomenclatorsByDescriptionContains;
                this.loading = false;
                resolve({
                  items
                })
          ...
          return new Promise((resolve, reject) => {
            ...
            this.$apollo.queries.nomenclators.fetchMore({
              ...
              updateQuery: (previousResult, {fetchMoreResult}) => {
                const items = fetchMoreResult.nomenclatorsPaging;
                this.loading = false;
                resolve({
                  items
                })
