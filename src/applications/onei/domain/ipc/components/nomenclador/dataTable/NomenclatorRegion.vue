<template>
  ...
  >
    <template slot="items" slot-scope="props">
      ...
      <td class="text-xs-left">{{ $t('applications.onei_ipc.modules.nomenclador.fields.'+props.item.activo) }}</td>
      ...
      <td class="text-md-center justify-center layout px-0">
        <v-menu bottom
          ...
          <v-list>
            ...
            <v-list-tile :to="`/onei/sigip/ipc/nomenclador/editar/tipo/region/${props.item.id}`">
              <v-list-tile-action>
                <v-icon>edit</v-icon>
              </v-list-tile-action>
              <v-list-tile-title>
            ...
            <v-list-tile :to="`/onei/sigip/ipc/nomenclador/view/tipo/region/${props.item.id}`">
              <v-list-tile-action>
...
<script lang="ts">
  ...
  import {
    QUERY_GET_REGION_PAGING,
    SEARCH_REGION_BY_DESCRIPTION,
    TOTAL_SEARCH_REGION_BY_DESCRIPTION
  ...
  export default {
    name: "onei-nomenclador-list-region",
    ...
    apollo: {
      nomenclators: {
        query: QUERY_GET_REGION_PAGING,
        variables: {
          page: 0,
          size: 5
        },
        update(data) {
          return data.regionesPaging
        }
      },
      totalNomenclators: {
        query: gql`query{
        ...
        update(data) {
          return data.totalRegiones
        }
      },
      nomenclatorsFilter: {
        query: SEARCH_REGION_BY_DESCRIPTION,
        variables: {
          descripcion: "null",
          page: 0,
          size: 5
        ...
        update(data) {
          return data.regionByDescripcion
        }
      },
      totalNomenclatorsFilter: {
        query: TOTAL_SEARCH_REGION_BY_DESCRIPTION,
        variables: {
          descripcion: "null",
        },
        update(data) {
          return data.totalRegionByDescripciones
        }
      }
    },
    watch: {
        ...
        if (this.filterModel.nombre !== '') {
          return new Promise((resolve, reject) => {
            ...
            this.$apollo.queries.totalNomenclatorsFilter.refetch({
              descripcion: this.filterModel.nombre
            });
            this.$apollo.queries.nomenclatorsFilter.fetchMore({
              ...
              updateQuery: (previousResult, {fetchMoreResult}) => {
                const items = fetchMoreResult.regionByDescripcion;
                this.loading = false;
                resolve({
                  items
                })
          ...
          return new Promise((resolve, reject) => {
            ...
            this.$apollo.queries.nomenclators.fetchMore({
              ...
              updateQuery: (previousResult, {fetchMoreResult}) => {
                const items = fetchMoreResult.regionesPaging;
                this.loading = false;
                resolve({
                  items
                })
