    ...
    <template slot="items" slot-scope="props">
      ...
      <td class="text-md-center justify-center layout px-0">
        <v-menu bottom
          ...
          <v-list>
            <v-list-tile :to="`/onei/sigip/ipc/nomenclador/editar/tipo/moneda/${props.item.id}`">
              <v-list-tile-action>
                <v-icon>edit</v-icon>
              </v-list-tile-action>
              <v-list-tile-title>
            ...
            <v-list-tile :to="`/onei/sigip/ipc/nomenclador/view/tipo/moneda/${props.item.id}`">
              <v-list-tile-action>
...
<script lang="ts">
  ...
  import {
    QUERY_GET_CURRENCY_PAGING,
    SEARCH_CURRENCY_BY_DESCRIPTION,
    TOTAL_SEARCH_CURRENCY_BY_DESCRIPTION
  ...
  export default {
    name: "onei-nomenclador-list-moneda",
    ...
    apollo: {
      nomenclators: {
        query: QUERY_GET_CURRENCY_PAGING,
        variables: {
          page: 0,
          size: 5
        },
        update(data) {
          return data.currencyPaging
        }
      },
      totalNomenclators: {
        query: gql`query{
        ...
        update(data) {
          return data.totalCurrency
        }
      },
      nomenclatorsFilter: {
        query: SEARCH_CURRENCY_BY_DESCRIPTION,
        variables: {
          description: "null",
          page: 0,
          size: 5
        ...
        update(data) {
          return data.currencyByDescriptionContains
        }
      },
      totalNomenclatorsFilter: {
        query: TOTAL_SEARCH_CURRENCY_BY_DESCRIPTION,
        variables: {
          description: "null",
        },
        update(data) {
          return data.totalCurrencyByDescriptionContains
        }
      }
    },
    watch: {
        ...
        if (this.filterModel.nombre !== '') {
          return new Promise((resolve, reject) => {
            ...
            this.$apollo.queries.nomenclatorsFilter.fetchMore({
              ...
              updateQuery: (previousResult, {fetchMoreResult}) => {
                const items = fetchMoreResult.currencyByDescriptionContains;
                this.loading = false;
                resolve({
                  items
                })
          ...
          return new Promise((resolve, reject) => {
            ...
            this.$apollo.queries.nomenclators.fetchMore({
              ...
              updateQuery: (previousResult, {fetchMoreResult}) => {
                const items = fetchMoreResult.currencyPaging;
                this.loading = false;
                resolve({
                  items
                })
