...
<script lang="ts">
  ...
  export default {
    name: "onei-reporte-cantidad-variedades-dpa",
    components: {
      VLayout,
      VFlex,
      VForm,
    ...
    watch: {
      ...
      dia_captar(val) {
        this.filtrodiacaptar = !!(val != null && val != -1);
      },
    },
    data() {
      return {
        ...
        dia_captar:-1,
        filtrodpa: false,
        filtrodiacaptar: false,
        linear_active: false,
        formatOutput: '.xls',
        availableFormats: [{label: 'PDF', value: '.pdf'}, {label: 'Excel', value: '.xls'}],
        ruta: '/Reportes_SIGIP/Reportes/Reporte_Cantidad_de_variedades_por_DPA',
      };
    },
    methods: {
      buscar() {
        if (!this.filtrodpa && !this.filtrodiacaptar) {
          ...
          const params =
            '?dpa=' + [this.dpa]
            + '&diacaptar=' + [this.dia_captar]
            + '&filtrodpa=' + this.filtrodpa
            + '&filtrodiacaptar=' + this.filtrodiacaptar
          JasperAPIBase
